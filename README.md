...
激光打在靶上，产生的负离子（离子阱可以对负离子降温）经过质谱仪筛选出目标离子团簇，加速并聚焦至成像透镜。脱附激光在成像透镜的前两个电极间的区域和离子团簇作用，产生因激光脱附的电子。之后加速并击中 MCP 并在 CCD 上成像 [1,2]。
...
> 仓库中提供了两个 DEMO（不保证完全符合题目），其中包含存储，读取 HDF5 和保存图片至 PDF 中的示例。如果需要的话，你可以参考并填充其中的必要部分。当然你可以自己重新实现一份代码，图片存储格式不限于PDF，你可以自由选择。

注意：考虑到体积因素，**不允许**将生成的任何数据文件提交到仓库中。如果你认为程序运行时间可能较长，可以在报告中提供预先生成的结果文件的下载方式（如清华云盘地址）。也建议你将一些有代表性的结果图插入到实验报告中，以便助教批阅。

#### ET-2 MCP倍增 40'

假定一个电子入射到第一片MCP上，对应位置为参考原点(0,0)

+ MCP表面和通道的面积比为表面积:孔面积=**4:6**。入射在MCP表面的电子在`ET-2`中认为损失掉了，可以按照0.6的Bernoulli分布描述(这个不影响动能分布，只会影响最后产生的电子数目)。
+ 在`ET-2`中认为产生的二次电子数目 N 服从泊松分布$\pi(4)$。每个电子的能量为入射电子动能的平均$\frac{E_k}{N}$。二次电子到下次碰撞管壁之前在电场作用下能量增加20eV。
+ 二次电子的倍增次数：在`ET-2`中认为倍增次数 G 服从泊松分布$\pi(7)$

从上面的过程中可以得到一个电子倍增后的电子动能分布，此外在`ET-2`中认为出射的二次电子角度分布为$U(0.004,1)$,这个分布决定了从第一片MCP出射的电子如何打在第二片MCP上。

电子的动能分布可以按照你的思考存储起来以便后续使用(你应该会注意到出射的电子动能最后与入射电子的动能几乎无关)。**如果你能推出串级随机变量的分布以及动能最后趋近的分布公式，可以直接使用公式采样**。

从第一层 MCP 出射后，大量电子会在两层 MCP 之间的间隙飞行，间隙距离为 $d_1=10^{-5}$，匀速飞行后电子入射第二层 MCP。在第二片MCP中的倍增过程和上述第一片MCP类似(你可以直接使用上面得到的倍增数目分布和动能分布采样作为第二次倍增的结果)。

1. `single_mcp.py`: 模拟单层MCP倍增电子过程，并存储模拟的结果，包括出射的电子动能。因为需要一个分布，所以最后存储的电子信息个数需不少于10000个。(这个倍增过程可以合并到`dual_mcp.py`，因为按照`ET-2`假设此处动能不会影响到出射角度分布)
2. `dual_mcp.py`: 根据单层MCP的模拟结果计算双层mcp的倍增结果分布信息，包括出射电子的动能和**出射位置**(你可以将两个MCP过程合并在该脚本中模拟，这样就不需要`single_mcp.py`,但是需要考虑计算量，如果有优化技巧可以写入报告中)。
...
1. `ccd.py`：输入`ET-1`的电子位置和`ET-2`MCP响应，模拟荧光屏上的电子击中位置，以及CCD上的计数。
2. `read_ccd.py`：绘制荧光屏上的电子位置分布图和CCD上的图像。

### 中级任务（Intermediate Task）10'

该部分细致考虑电子的倍增过程，对应于 `ET-2`。

+ 电子入射在 MCP 表面时会产生多个二次电子并再次进入孔洞，二次电子的动能低于 3eV 认为不再产生二次电子
+ 电离产生的二次电子能量分布为指数分布 $E\mathrm{Exp}(E, 7eV)$。每个二次电子的能量不相同，所以在采样得到二次电子的数目之后，采样二次电子动能时必须考虑 N 个二次电子能量之和小于入射电子能量。电离期望的二次电子数目 N 服从泊松分布 $\pi(4)$。

### 高级任务（Advanced Task）10'

该部分细致考虑二次电子的出射角度，对应于 `ET-3`。

+ 认为产生的二次电子的过程分为 2 类：弹性散射，电离。概率比例为 0.85 和 0.15。
...
+ 出射的二次电子角度分布遵从：$\cos^3\theta$ ~ $U(0,1)$，$\phi$ ~ $U(0,2\pi)$。
+ MCP 的通道视作一个空心圆柱，你需要计算电子在圆柱通道内的运动，并计算倍增的次数。通道的直径为 6um，厚度为 60um。MCP 两端的电压为 400V。

### Makefile

本次作业提供了 `Makefile`，最终助教也将使用 `Makefile` 进行测试。需要注意，你在编写所有程序文件时，都应该使用 `make` 给程序传入的参数（来自 `sys.argv`，DEMO程序中用了 `argparse`），而非硬编码下面提到的文件名；否则，你可能无法通过测试。

在本目录中运行 `make -n` 即可看到实际运行的命令，这或许能帮助你开发。

`Makefile` 中的 `all` 依赖项为必须生成的文件（`VelocityImage.h5 DualGain.h5 CCDImage.h5 VelocityImage.pdf DualGain.pdf CCDImage.pdf`），你可以根据需要进行修改。

### 非功能要求（20'）

此部分请参照大作业公告。如果你使用了第三方的软件，特别是非自由软件，请务必熟悉其条款。如果第三方代码规定未经授权不得使用，那么使用时请取得授权。

### 其他说明

`ET` 是基础部分（70'），如果你对MCP的倍增过程有兴趣可以选择 `IT`（10'）和 `AT`（10'）部分。

`ET-2` 为今年新增部分，如果无法顺利完成 `ET-2` 可以使用简单的角分布和增益公式作为 `ET-3` 的输入，不影响 `ET-3` 的评分。
