...
+ 出射的二次电子角度分布各向同性，$\cos^3\theta$ ~ $U(0,1)$，$\phi$ ~ $U(0,2\pi)$。
+ MCP的通道视作一个空心圆柱，你需要计算电子在圆柱通道内的运动，并计算倍增的次数。通道的直径为6um，厚度为60um。MCP两端的电压为400V。
...
`ET`是基础部分(85')，如果你对MCP的倍增过程有兴趣可以选择`IT`(10')和`AT`(10')部分。
